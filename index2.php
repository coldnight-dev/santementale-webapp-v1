...
$packageVersions = ['1.0', '1.1'];
// Vérification de la version passée en GET ou dans localStorage
$clientVersion = $_GET['v'] ?? null;
$storedVersion = isset($_COOKIE['client_version']) ? $_COOKIE['client_version'] : null;
if (!$clientVersion && !$storedVersion) {
    http_response_code(403);
    exit('403 Forbidden');
}
// Si une version est passée en GET, elle prime et est stockée
if ($clientVersion && preg_match('/\A1\.[0-9]{1,2}|1\.web\z/', $clientVersion)) {
    $currentVersion = $clientVersion;
    setcookie('client_version', $currentVersion, time() + (30 * 24 * 60 * 60), "/");
} elseif ($storedVersion && in_array($storedVersion, $packageVersions)) {
    $currentVersion = $storedVersion;
} else {
    http_response_code(403);
    exit('403 Forbidden');
}
$isPackageVersion = in_array($currentVersion, $packageVersions);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        :root {
            ...
            --footer-color: #555555;
            --pencil-fill: #333333;
            --gray-btn: #555555; /* For gray settings button */
        }
        * {
            margin: 0;
            padding: 0;
        ...
        @keyframes pulse {
            0% { transform: scale(1); }
            50% { transform: scale(1.0363); } /* Amplified by 10% from 1.033 */
        ...
        .cta {
            padding: 16px 16px 16px 12px; /* Adjusted padding: less on left for icon alignment */
            font-size: clamp(14px, 4vw, 16px);
            background: var(--gradient-blue);
            color: var(--text-primary);
            border: none;
        ...
        .cta .material-icons {
            margin-right: 12px; /* Increased spacing for better balance */
            font-size: 20px;
...
<body>
    ...
    <div id="aboutPopup" class="popup">
        <div class="popup-content">
            ...
            <p>API : 0.dev</p>
            <p>App : <?php echo htmlspecialchars($currentVersion); ?></p>
            <p id="clientUUID"></p>
    ...
    <div class="container">
        ...
        <p class="footer" style="margin-top: 3em; color: var(--footer-color); line-height: 1.8; font-size: 14px;">
            v<?php echo htmlspecialchars($currentVersion); ?> • Accès anticipé<br>
    ...
    <script>
        if (localStorage.getItem('device_uuid') && !localStorage.getItem('username')) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let clientVersion = '<?php echo htmlspecialchars($currentVersion); ?>';
            if (clientVersion && clientVersion.match(/\A1\.[0-9]{1,2}|1\.web\z/)) {
                localStorage.setItem(clientVersionKey, clientVersion);
            } else {
                clientVersion = localStorage.getItem(clientVersionKey);
                if (!clientVersion || !clientVersion.match(/\A1\.[0-9]{1,2}|1\.web\z/)) {
                    console.log('Aucune version valide détectée.');
                }
            }
            const isInstalled = window.matchMedia('(display-mode: standalone)').matches || navigator.standalone === true;
            const isPackageVersion = <?php echo json_encode($isPackageVersion); ?>;
            ...
            if (isAndroid) {
                ...
                bannerIcon.innerHTML = '<span class="material-icons banner-icon">apple</span>';
            }
            function showInstallPopup() {
                ...
                if (isAndroid && deferredPrompt) {
                    ...
                    installBtn.textContent = 'Installer le raccourci';
                    installBtn.classList.add('cta');
                    installBtn.onclick = () => {
                    ...
                    instructions.textContent = "Pour installer : Appuyez sur l'icône de partage en bas de l'écran, puis sélectionnez 'Ajouter à l'écran d'accueil'.";
            ...
            if (!isPackageVersion && !isInstalled) {
