...
function renderIconPaginationControls(iconArray, currentPage, onPageChange, containerId) {
    ...
    if (totalPages > 1) {
        ...
        html += '<button class="icon-pagination-btn" ' + (currentPage === totalPages - 1 ? 'disabled' : '') + ' onclick="' + onPageChange + '(1)"><span class="material-symbols-outlined" style="font-size:18px;">chevron_right</span></button>';
    }
...
function render() {
    ...
    if (state.view === 'today') {
        ...
        h += '<button onclick="generateShareImage()" class="w-full py-3 text-white font-bold rounded-lg flex items-center justify-center gap-2" style="transition:all 0.2s;background:linear-gradient(135deg,#0d47a1 0%,#1976d2 100%);"><span class="material-symbols-outlined">share</span>Partager</button>';
        ...
        h += '<div class="text-center p-3 bg-yellow-950 rounded-lg border border-yellow-900"><div class="text-4xl font-bold text-yellow-400">' + state.perfectDaysCount + '</div><div class="text-xs text-zinc-500">Journées parfaites</div></div></div></div>';
        let totalMinutes = 0;
        state.history.forEach(day => state.routines.forEach(r => r.tasks.forEach(t => {
        ...
        h += '<div class="text-sm text-purple-300 mt-1">Temps total consacré à vos routines</div></div></div>';
        renderStatsChart();
        ...
        h += '<div class="text-center"><div class="text-6xl font-bold text-orange-500 mb-2' + (state.streak > 0 ? ' streak-pulse' : '') + '">' + state.streak + '</div><div class="text-zinc-400">jour' + (state.streak > 1 ? 's' : '') + ' à >75%</div></div></div>';
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        let dayHistory = state.history.find(h => h.date === today);
        if (dayHistory) {
            state.routines.forEach(r => r.tasks.forEach(t => t.completed = !!(dayHistory.routines[r.id] && dayHistory.routines[r.id][t.id])));
        } else {
