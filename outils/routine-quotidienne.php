...
<head>
    ...
    <style>
        ...
        /* Animation pour l'icône du titre principal */
        @keyframes slow-spin {
            0%, 38.89% { transform: rotate(0deg); } /* Attente 7s */
            55.56% { transform: rotate(180deg); } /* Rotation 0° à 180° en 3s */
            83.33% { transform: rotate(180deg); } /* Attente 5s */
            100% { transform: rotate(360deg); } /* Rotation 180° à 360° en 3s */
        }
        .material-symbols-outlined.title-icon-animated {
            animation: slow-spin 18s linear infinite;
        }
        /* Animation pour les icônes des titres de routines */
        @keyframes float-drift {
            0%, 100% { transform: translateY(0); }
            50% { transform: translateY(-3px); }
        }
        .material-icons.routine-icon-animated {
    ...
    <script>
        ...
        function render() {
            ...
            state.routines.forEach(routine => {
                ...
                h += '<div class="text-center p-3 bg-zinc-950 rounded-lg"><div class="text-6xl font-bold">' + state.history.length + '</div><div class="text-xs text-zinc-500">Jours enregistrés</div></div>';
                h += '<div class="text-center p-3 bg-blue-950 rounded-lg border border-blue-900"><div class="text-6xl font-bold text-blue-400">' + state.streak + '</div><div class="text-xs text-zinc-500">Série (>75%)</div></div></div></div></div>';
                ...
                h += '<div class="text-6xl font-bold text-blue-500 mb-2 streak-pulse">' + state.streak + '</div><div class="text-zinc-400">jour' + (state.streak>1?'s':'') + ' à >75%</div></div></div>';
